...
namespace MFST
{
	...
	Mfst::Mfst(LT::LexTable plex, GRB::Greibach pgrebach)//(результат работы лексического анализатора, грамматика Грейбах)
	{
		...
		{
			lenta[i] = TS(lex.table[i].lexema);//заносит в ленту терминалы   *убрал TS(*lex.table[k].lexema);
		}
		lenta_position = 0;
	...
	Mfst::RC_STEP Mfst::step(const Log::LOG& log) //выполнить шаг автомата
	{
		...
		if (lenta_position < lenta_size)
		{
			...
			{
				MFST_TRACE4(log, "TS_NOK/NS_NORULECHIN") rc = reststate(log) ? TS_NOK : NS_NORULECHAIN;
			};
		}
		else if ((st.top() != '$'))
	...
	char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: строка %d,%s", err.id, lex.table[lpos].lexema, err.message);
			rc = buf;
		};
		return rc;
	};
