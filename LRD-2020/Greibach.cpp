...
namespace GRB
{
	Greibach greibach(NS('S'), TS('$'), // стартовый символ, дно стека
		...
		Rule(NS('M'), GRB_ERROR_SERIES + 3, // ошибка в арифметике
			2,  // oE | oEM 
			//Rule::Chain(2, TS('i'), NS('E')),
			Rule::Chain(2, TS('o'), NS('E')), //
			Rule::Chain(3, TS('o'), NS('E'), NS('M'))
		),
		Rule(NS('F'), GRB_ERROR_SERIES + 4,  // ошибка в формальных параметрах функции (прототипах)
			2,  // ti | ti,F 
			Rule::Chain(2, TS('t'), TS('i')),		//
			Rule::Chain(4, TS('t'), TS('i'), TS(','), NS('F'))		//
		),
		Rule(NS('W'), GRB_ERROR_SERIES + 5,  // ошибка в фактических параметрах функции (вызове)
			4,  // i | l | i,W | l,W
			Rule::Chain(1, TS('i')), //
			Rule::Chain(1, TS('l')), //
	...
	Greibach::Greibach(GRBALPHABET pstartN, GRBALPHABET pstbottom, short psize, Rule r, ...)//конструктор граматики Грейбаха(стартовый символ, дно стека, количество правил, правила...)
	{
